...
# ═══════════════════════════════════════
# Package Recommender Engine (853,111 PyPI index)
# ═══════════════════════════════════════


@router.get("/packages/recommend")
async def recommend_packages(q: str = "", limit: int = 10) -> dict[str, Any]:
    """
    Search and recommend Python packages from index of 853,111 PyPI packages.
    """
    from src.recommender.pypi_recommender import PyPIPackageRecommender

    try:
        rec = PyPIPackageRecommender()
        results = rec.recommend(query=q, limit=limit)
        return {"success": True, **results}
    except Exception as e:
        logger.error(f"Package recommendation error: {e}")
        raise HTTPException(status_code=500, detail=str(e))
