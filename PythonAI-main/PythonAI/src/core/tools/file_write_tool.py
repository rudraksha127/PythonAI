    ...
    type(
        ...
        (),
        {
            ...
            "get_activity_description": lambda input_data: (
                f"Writing {input_data.get('file_path', '')}" if input_data else None
            ),
        },
    )
)
...
def _write_validate(input_data: dict[str, Any], context: ToolUseContext) -> ValidationResult:
...
def _write_call(input_data: dict[str, Any], context: ToolUseContext) -> ToolResult:
    file_path = input_data.get("file_path", "")
    content = input_data.get("content", "")
    ...
    try:
        ...
        return ToolResult(data={"error": f"Failed to write file: {e}"}, error=f"Failed to write file: {e}")
