    ...
    type(
        ...
        (),
        {
            ...
            "get_activity_description": lambda input_data: (
                f"Searching for {input_data.get('query', '')[:40]}..." if input_data else None
            ),
        },
    )
)
...
def _search_validate(input_data: dict[str, Any], context: ToolUseContext) -> ValidationResult:
...
def _search_call(input_data: dict[str, Any], context: ToolUseContext) -> ToolResult:
    query = input_data.get("query", "")
