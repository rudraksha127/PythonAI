...
def _payload(system="sys", user="hi", tools=None):
    ...
    return llm_core._build_anthropic_payload(
        "claude", messages, 0.0, 1000, stream=True, tools=tools
    )
...
def test_agentic_caches_system_and_last_tool():
    tools = [
        ...
        {
            "type": "function",
            "function": {"name": "b", "description": "y", "parameters": {}},
        },
    ]
    ...
    breakpoints = sum("cache_control" in b for b in p["system"]) + sum(
        "cache_control" in t for t in p["tools"]
    )
