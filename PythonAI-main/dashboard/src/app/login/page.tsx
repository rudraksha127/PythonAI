...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch("http://localhost:7337/api/auth/login", {
        method: "POST",
    ...
    <div className="min-h-screen flex items-center justify-center px-4">
      <div className="w-full max-w-md">
        ...
        <form onSubmit={handleSubmit} className="glass rounded-xl p-6 space-y-4 border border-forge-border">
          ...
          <div className="relative my-4">
            ...
            <div className="relative flex justify-center text-xs">
              <span className="bg-forge-elevated px-2 text-text-muted">or continue with</span>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <button
              type="button"
              className="flex items-center justify-center gap-2 py-2.5 px-4 border border-forge-border rounded-lg text-text-secondary hover:text-text-primary hover:bg-forge-elevated transition-all"
              onClick={() => setError("SSO not configured yet. Use email/password.")}
            >
        ...
        <p className="text-center mt-6 text-sm text-text-muted">
          ...
          <Link href="/signup" className="text-forge-primary hover:underline font-medium">
            Sign up
          </Link>
        </p>
      </div>
