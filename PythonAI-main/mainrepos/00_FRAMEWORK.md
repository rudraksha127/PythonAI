...
M14 — Massive autonomous acquisition    (needs M2, feeds M10 — audit/activate
                                          an already-built 1600-source system,
                                          see note below)
```

A correction worth stating plainly, because it happened while building this
exact folder and is the clearest real example of technique #3 (verified
grounding) actually mattering: M1 originally did not mention that this
repository uses Git LFS. A plain clone leaves every LFS-tracked file as a
small pointer stub — `data/training/training_dataset.json` checked out at
133 bytes and looked empty, until reading the pointer file itself revealed
its real size is 41.5MB, sitting safely in LFS storage, never lost. M1 and
M2 have both been corrected to check for this before assuming any data is
missing. The lesson generalizes: a file that looks empty or missing is a
question to verify, not a fact to build a plan on top of.
