...
for cell in cells:
    source_str = "".join(cell.get("source", []))
    if "MODEL_NAME = \"unsloth/Qwen2.5-Coder-14B-bnb-4bit\"" in source_str and "FastLanguageModel.from_pretrained" in source_str:
...
for cell in cells:
    source_str = "".join(cell.get("source", []))
    if "trainer = SFTTrainer(" in source_str and "per_device_train_batch_size=2" in source_str and "qwen14b_pythonai_adapter" in source_str:
