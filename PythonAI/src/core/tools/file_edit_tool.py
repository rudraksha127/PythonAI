...
FileEditTool = build_tool(
    type("FileEditToolDef", (), {
        ...
        "get_activity_description": lambda input_data: f"Editing {input_data.get('file_path', '')}" if input_data else None,
    })
)


def _edit_validate(input_data: dict[str, Any],
...
def _edit_call(input_data: dict[str, Any],
    ...
    try:
        ...
        return ToolResult(data={"error": f"Edit failed: {e}"},
