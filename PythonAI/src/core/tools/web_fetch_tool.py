...
WebFetchTool = build_tool(
    type("WebFetchToolDef", (), {
        ...
        "get_activity_description": lambda input_data: f"Fetching {input_data.get('url', '')[:40]}..." if input_data else None,
    })
)


def _fetch_validate(input_data: dict[str, Any],
...
def _fetch_call(input_data: dict[str, Any],
    ...
    try:
        ...
        return ToolResult(data={"error": f"Failed to fetch {url}: {e}"},
